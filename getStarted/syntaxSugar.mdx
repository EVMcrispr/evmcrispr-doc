...
A common example of using the @now helper would be to create a vesting schedule using [Aragon's Token Manager App](/aragonOS/token-manager#internal-actions)

```
exec token-manager assignVested @me 1000e18 @now @calc(@now + 10d) @calc(@now + 30d) false
```
